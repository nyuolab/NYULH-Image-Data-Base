...
def create_dicom_tar(source_dir, output_dir):
    ...
    for root, dirs, files in os.walk(source_dir):
        ...
        for file in tqdm(files):
            ...
            with tarfile.open(f"{output_dir}/tar_gz/{out_file_name}.tar.gz", "w:gz") as tar_compress:
                file_path = os.path.join(root, file)
                if file.endswith('.dcm') or os.path.isfile(file_path):
                    # Add the file to the tar archive
                    t0 = time.perf_counter()
     
                    tar_compress.add(file_path, arcname=os.path.basename(file_path))
                    t_tar_compress = time.perf_counter()

                
                    delta_tar_compress = t_tar_compress - t0 

                    total_tar_compress += delta_tar_compress
